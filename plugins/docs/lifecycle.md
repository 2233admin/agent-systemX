...
| L1／L2／L3 与维护面必须可复现 | 2 拦截 | CI 递归实测 `complexityBudget` 的三层计量 |
| Skill 数量门已退役，不得重新引入 | 2 拦截 | CI 断言 `retiredSkillCountGate` 记录完整且无 `maxSkills` |
| 来源退役必须确认目录、两份 Marketplace 与当前路由都不存在 | 2 拦截 | CI 负例 |
| 运行端退役必须逐端卸载并回读 | **4 文本** | 来源仓无法代替目标客户端回执 |
| 开发不在生产检出里做 | **4 文本** | 无（只影响漂移检测能否判定，不影响会话行为） |
| 被标记 `suspect` 后不得再作判据 | **4 文本** | 无 |
| 写作判据（leading word／完成门／退出） | **4 文本** | 无 |

四条停在第 4 层的规则是本文的已知弱点，不是遗漏。它们上移的条件分别是：有可访问的目标客户端回执；恢复能识别生产检出的发布前置；找到「作为判据使用」的可观察信号；找到不制造合规假象的完成门判据。
