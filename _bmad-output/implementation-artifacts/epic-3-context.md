...
- Story 3.5：从 Skill 供给库产出配置修订候选（configs supply）— done
...
- **字节落点不得踩原生发现路径**，否则唯一装配路径退化为无人执行的声明。**2026-08-25 更正（实跑安装器所得）：** 原写「canonical 落点定为 `_bmad/`，安装器须配置为不产生 IDE 投影」——实跑证实安装器不会把 skill 本体写进 `_bmad/`，投影是它唯一的产出形式，该手段做不到。落点规则本身不变，满足它的手段待裁定（`--list-tools` 显示部分 tool 目标目录并非 Claude／Codex 发现路径，是唯一已知候选，未验证）。
...
- **3.5 已在 3.4 之后交付并完成验证。** 顺序约束已满足，供应候选经既有建立路径消费；后续保留的供应根与外部来源问题不改变 Story 3.5 已完成的状态。
...
- **本仓 Skill 资产退役严格三步、不得倒序**：(1) 落地替代——**（2026-08-25 更正：不存在「恢复 canonical」这项前置——实跑安装器证实 skill 本体从来就不落 `_bmad/`，没有东西被删；`configs` 的物化来源就是投影目录本身，实测 `--group .agents` 可扫到全部 49 个）**；(2) parity 验证——经 `configs use` 启动的会话，其实际可用 Skill 集合与原生发现所得集合做**真实烟雾对照**（非清单比对），且必须在不存在用户级安装的环境中进行，否则结论假阳性；(3) 退役——原生发现目录改为非跟踪并进入忽略清单，双份字节一致门随之退役。第 (2) 步稳定前两份保持跟踪、一致门继续生效。
