...
- **Rule：** 产品装配面（消费者是 `configs` 的用户）与本仓自我开发装配面（消费者是在本仓工作的 agent 会话）是两个不同的**消费者**，但共用同一条装配路径与同一套合同——不为后者另立机制。
  ...
  - **字节的落点不得是原生发现路径。** 上一条要求每机复现字节，本条约束它们落在哪：**不得落在任何客户端的原生发现路径上**（当前已知即 `.claude/skills/`、`.agents/skills/`）。否则会话不经 `configs` 也照样拿到 Skill，"唯一装配路径"就退化成一句无人执行的声明——与 `entrypoints/agent-system.md` 那三条"加载可用的 X"把硬门降级为空操作是同一种失败模式，本条要防的正是它重演。canonical 落点取 `_bmad/`：它不是任何客户端的原生发现路径，且 `_bmad/_config/skill-manifest.csv` 的 `path` 列本就指向 `_bmad/core/<skill>/SKILL.md`——这是 BMad 自身的安装形状，不是本条新发明。安装器须配置为不产生任何 IDE 投影，`configs` 从 canonical 路径物化。**根本身怎么取，两种部署场景都要认：** 本仓自我开发时，供给库就是本仓库，根经 `CONTROL_PLANE_SUPPLY_ROOT` 指向仓库根（否则 `_bmad/core/<skill>` 这类库内相对路径解析不到）；发行版 `configs` 用户的机器上没有「本仓库」这个东西，走 `defaultSupplyRoot()` 的默认值 （`path.dirname(defaultDbPath())` 派生的 `supply/`）。默认值只服务后者——不把它改成仓库根派生，是因为编译后的二进制没有仓库可派生，绕一圈仍要回退到某个 `$HOME` 约定。**在本仓可控面内，规则由机制强制而非自觉。**已知的外部限制：客户端同样会从**用户级／机器级** skill 目录发现 Skill，那不是项目目录，本仓的 `.gitignore` 与安装器 `ides` 配置都管不到——某台机器若在用户级装了 BMad，其会话不经 `configs` 也会拿到 Skill。本仓无权约束他人机器，不试图去管；但第 (2) 步的 parity 验证**必须在不存在用户级 BMad 安装的环境中进行**，否则结论是假阳性。（本机 2026-08-25 实测：`~/.claude/skills` 与 `~/.agents/skills` 均不存在。）
  ...
  - **规则与能力必须同真。** 仓库规则（`entrypoints/agent-system.md` 及其投影）不得强制加载一个本仓当前装配下不可获得的 Skill。任一时点二者必须一致：要么该 Skill 进入本仓装配意图，要么该规则改写或删除。"加载可用的 X"这类措辞**不构成豁免**——它把硬门静默降级为空操作，正是本条要防的失败模式。当前已知不一致：`entrypoints/agent-system.md` 第 62 行要求的 `orchestrated-collaboration`、第 78／84 行要求的 `adaptive-problem-solving`（后者含"父目标验收"硬门）都只存在于 `plugins/`，不在任何客户端的原生发现路径上，对本仓会话结构性不可见；退役第 (1) 步必须把它们纳入本仓装配意图，否则该步不算完成。
...
- **自我开发装配一致性（AD-22，退役第 (1) 步落地后生效的待建门，不是现行门）：**在 AD-22 退役顺序第 (1) 步完成之前，本仓处于该条明确允许的过渡态（规则强制的三个 Skill 尚不可见、两份投影仍被跟踪），此门不适用；第 (1) 步完成即随之生效。届时机械校验"仓库规则强制加载的每个 Skill 都在本仓当前装配意图内"——`entrypoints/agent-system.md` 及其投影中出现的每个 Skill 名，必须能在本仓装配意图里解析到一个真实来源；解析不到即失败，不接受"加载可用的 X"措辞作为豁免。退役第 (2) 步的 parity 验证是一次性真实烟雾对照（经 `configs use` 启动的会话实际可用 Skill 集合 vs 原生发现集合），不是清单比对，也不是长期双运行。
