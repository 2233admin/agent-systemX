...
- **Rule：** 产品装配面（消费者是 `configs` 的用户）与本仓自我开发装配面（消费者是在本仓工作的 agent 会话）是两个不同的**消费者**，但共用同一条装配路径与同一套合同——不为后者另立机制。
  ...
  - **字节的落点不得是原生发现路径。** 上一条要求每机复现字节，本条约束它们落在哪：**不得落在任何客户端的原生发现路径上**（当前已知即 `.claude/skills/`、`.agents/skills/`）。否则会话不经 `configs` 也照样拿到 Skill，"唯一装配路径"就退化成一句无人执行的声明——与 `entrypoints/agent-system.md` 那三条"加载可用的 X"把硬门降级为空操作是同一种失败模式，本条要防的正是它重演。**2026-08-25 更正（实跑安装器所得，取代此前的推断）：** 本条原写「canonical 落点取 `_bmad/`……安装器须配置为不产生任何 IDE 投影」，依据是 `_bmad/_config/skill-manifest.csv` 的 `path` 列指向 `_bmad/core/<skill>/SKILL.md`。该依据不成立——实跑 `bmad-method@6.11.0 install --action update` 证实：安装器**不会**把 skill 本体写进 `_bmad/`，`files-manifest.csv` 记的是上游包内有哪些文件，不是安装器会往 `_bmad/` 落哪些文件；`_bmad/` 下实际只有配置、清单与脚本（263 条中仅 15 条落地），skill 本体按设计**只落 IDE 投影目录**。因此「配置安装器不产生 IDE 投影」是做不到的——投影是它唯一的产出形式，`--tools` 只能选投影到哪些目录。**本条的落点规则本身不变**（字节不得落在客户端的原生发现路径上），但满足它的手段待定：`--list-tools` 显示部分 tool 的目标目录并非 Claude／Codex 的发现路径（如 `adal → .adal/skills`、`cline → .cline/skills`），用这类 ID 安装可让字节落在中立位置，是当前唯一已知的候选手段，尚未验证，也尚未裁定是否采纳。**根本身怎么取，两种部署场景都要认：** 本仓自我开发时，供给库就是本仓库，根经 `CONTROL_PLANE_SUPPLY_ROOT` 指向仓库根（否则 `_bmad/core/<skill>` 这类库内相对路径解析不到）；发行版 `configs` 用户的机器上没有「本仓库」这个东西，走 `defaultSupplyRoot()` 的默认值 （`path.dirname(defaultDbPath())` 派生的 `supply/`）。默认值只服务后者——不把它改成仓库根派生，是因为编译后的二进制没有仓库可派生，绕一圈仍要回退到某个 `$HOME` 约定。**在本仓可控面内，规则由机制强制而非自觉。**已知的外部限制：客户端同样会从**用户级／机器级** skill 目录发现 Skill，那不是项目目录，本仓的 `.gitignore` 与安装器 `ides` 配置都管不到——某台机器若在用户级装了 BMad，其会话不经 `configs` 也会拿到 Skill。本仓无权约束他人机器，不试图去管；但第 (2) 步的 parity 验证**必须在不存在用户级 BMad 安装的环境中进行**，否则结论是假阳性。（本机 2026-08-25 实测：`~/.claude/skills` 与 `~/.agents/skills` 均不存在。）
