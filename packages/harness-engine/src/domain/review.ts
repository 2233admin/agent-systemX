...
export function validateResidualClosure(value: unknown): value is ResidualClosure {
  ...
  if (Object.keys(candidate).length !== 4 || Object.keys(candidate).some((key) => !['owner', 'decision', 'target', 'closureEvidence'].includes(key))
    || !nonEmpty(candidate.owner) || !nonEmpty(candidate.decision) || !nonEmpty(candidate.target)) return false;
...
export function validateReviewPackage(value: unknown): value is ReviewPackage {
  if (typeof value !== 'object' || value === null || Array.isArray(value)) return false;
  const candidate = value as Record<string, unknown>;
  const allowed = ['planId', 'taskId', 'baseSha', 'headSha', 'path', 'createdAt'];
  return Object.keys(candidate).every((key) => allowed.includes(key))
    && Object.keys(candidate).length === allowed.length
    && allowed.every((key) => Object.hasOwn(candidate, key))
    && nonEmpty(candidate.planId)
...
export function createReviewPackage(input: ReviewPackage): ReviewPackage {
  if (!validateReviewPackage(input)) {
    throw new TypeError('ReviewPackage requires only six declared fields with valid values');
  }
  return {
    planId: input.planId,
    taskId: input.taskId,
