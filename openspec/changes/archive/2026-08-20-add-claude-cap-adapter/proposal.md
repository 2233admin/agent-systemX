> 方案审阅载体：[#81](https://github.com/Eridanus117/agent-system/issues/81)。阻断项 [#82](https://github.com/Eridanus117/agent-system/issues/82)（lock mode 跨平台缺陷）。关联 [#83](https://github.com/Eridanus117/agent-system/issues/83)（Windows ACL 校验缺口，不阻断）。
...
### 决定 1：认证模式默认值 —— ✅ 已确认：订阅 OAuth（2026-08-19，[决定记录](https://github.com/Eridanus117/agent-system/issues/81#issuecomment-5349961040)）
...
### 决定 2：是否先修 lock 的跨平台缺陷 —— ✅ 已确认：先修（2026-08-19，[决定记录](https://github.com/Eridanus117/agent-system/issues/81#issuecomment-5349961040)），且已实施
...
它阻断本变更「lock 中其他客户端 hash 不变」这一验收标准的判读。**已按负责人决定先行修复并验证**：Windows 上 `cap lock` 现在产出与 Linux 提交版逐字节一致的 lock。详见 `tasks.md` 任务 0.1 与 [#82](https://github.com/Eridanus117/agent-system/issues/82)。

### 决定 3：证据天花板的表述 —— ✅ 已确认：照写不弱化（2026-08-19，[决定记录](https://github.com/Eridanus117/agent-system/issues/81#issuecomment-5349961040)）
